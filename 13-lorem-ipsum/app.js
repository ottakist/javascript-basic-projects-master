...
form.addEventListener('submit',(e)=>{
  ...
  getRandomStrings(amount.value)
  
})
const getRandomStrings=(amount)=>{
  // console.log(this.amount.value)
  if(this.amount.value>0){
  for(let i = 0;i<=amount-1;i++){
  
  let p = document.createElement("p")
  p.innerHTML=`<p class="result">${text[getRandom()]}</p>`
  article.append(p)
}
  }
  else{
    article.innerHTML=`<p class="result">${text[getRandom()]}</p>`
  }
}
const getRandom=()=>{
